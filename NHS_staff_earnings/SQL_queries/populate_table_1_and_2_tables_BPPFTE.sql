...
-- Calculate BPPFTE for all staff 
select
[TM_END_DATE]
,'{AREA}'
,'Table 1 & 2'
...
,'All staff'
,'{PAYMENT_TYPE}'
,round(sum({PAYMENT_TYPE})/(count([UNIQUE_NHS_IDENTIFIER])),2,0),
count([UNIQUE_NHS_IDENTIFIER])
from TMP_CUTDOWN_EARNINGS_{AREA}_{YEAR}{MONTH}{USER} a
where {PAYMENT_TYPE} is not null
group by [TM_END_DATE]

insert into TMP_EARNINGS_TABLE1_AND_2_STAGING_{AREA}_{YEAR}{MONTH}{USER}
-- Calculate BPPFTE by Main Staff Group
select
[TM_END_DATE]
,'{AREA}'
,'Table 1 & 2'
...
insert into TMP_EARNINGS_TABLE1_AND_2_STAGING_{AREA}_{YEAR}{MONTH}{USER}
-- Calculate BPPFTE by Staff Group
select
[TM_END_DATE]
,'{AREA}'
,'Table 1 & 2'
...
from TMP_CUTDOWN_EARNINGS_{AREA}_{YEAR}{MONTH}{USER} a
where {PAYMENT_TYPE} is not null
and [MAIN_STAFF_GROUP_NAME] not like 'Other staff or those with unknown classification'
group by [TM_END_DATE],[STAFF_GROUP_1_NAME]

insert into TMP_EARNINGS_TABLE1_AND_2_STAGING_{AREA}_{YEAR}{MONTH}{USER}
-- Calculate BPPFTE by grade (HCHS doctors only)
select
[TM_END_DATE]
,'{AREA}'
,'Table 1 & 2'
